...
STATIC INLINE int RingBuffer_Drop(RINGBUFF_T *RingBuff, int num)
{
	register int count = RingBuffer_GetCount (RingBuff);
	RingBuff->head -= num < count ? num : count;
...
/**
 * @brief	Return data store of ring buffer
 * @param	RingBuff	: Pointer to ring buffer
 * @return	Pointer to the ring buffer storage area
 */
STATIC INLINE unsigned char * RingBuffer_GetDataPtr(RINGBUFF_T *RingBuff)
{
	return ((unsigned char *) RingBuff->data);
}
...
/**
 * @brief	Get a specific item from the ring buffer
 * @param	RingBuff	: Pointer to ring buffer
 * @param	data		: Pointer to memory where item be stored
 * @param	itemnum		: Position of the item to fetch
...
/**
 * @brief	Get specific items from the ring buffer
 * @param	RingBuff	: Pointer to ring buffer
 * @param	data		: Pointer to memory where item be stored
 * @param	firstItemNum		: Position of the first item to fetch
 * @param	numItems	: Number of item to fetch
 * @return	1 when item popped successfuly onto @a data,
 * 			0 When error (Buffer not initialized using
 * 			RingBuffer_Init() or attempted to pop item when
 * 			the buffer is empty)
 */
int RingBuffer_GetItemS(RINGBUFF_T *RingBuff, void *data, uint32_t firstItemNum, uint32_t numItems);
