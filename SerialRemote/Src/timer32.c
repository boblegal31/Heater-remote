...
void init_timer32(uint8_t timer_num, uint32_t TimerInterval) 
{
  if ( timer_num == 0 )
  {
    ...
    timer32_0_counter = 0;
#endif //TIMER32_0_DEFAULT_HANDLER
    LPC_TMR32B0->TCR = 2;	/* Reset timer, then clear timer count */
    LPC_TMR32B0->TC = 0;

    LPC_TMR32B0->PR = TimerInterval;	/* Setup prescaler and reset prescaler count */
    LPC_TMR32B0->PC = 0;


    LPC_TMR32B0->CCR = 7;	/* Capture on both edges and interrupt */

	LPC_TMR32B0->EMR = 0;
    LPC_TMR32B0->MCR = 0;	/* No match interrupt */
	LPC_TMR32B0->CTCR = 0;	/* Run in timer mode */
